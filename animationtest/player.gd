...
func _on_event_detect_area_entered(area: Area2D) -> void:
	...
	if area.name == "tiger":
		...
		score += 25
		area.queue_free()
		if area.name == "tiger" and $Gadget/AnimationPlayer.current_animation == "jump":
			print("tiger killed")
			area.get_parent().emit_signal("defeated")
			score += 100
			velocity.y = -300
			updatevalues()
		elif area.name == "tiger" and velocity.y <= 0 and invincible == false:
			hurt()
			updatevalues()
			print("tiger should hurt player")
